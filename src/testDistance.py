...
for i in range(num_samples):
	dis_img = test_random_transform(image_in=image, min_scale=0.25, max_scale=1.0, max_rotation=0)
	dis_logits, _ = inception(dis_img, reuse = True)
	average_loss += tf.nn.softmax_cross_entropy_with_logits(logits=dis_logits, labels = labels) / num_samples
...
demo_lr2 = 2e-1
demo_steps2 = 20 # small scale picture is hard to be trained into an adversarial sample
demo_target2 = 924
...
for i in range(0, 10):
	dis_out = test_random_transform(image_in=image, min_scale=0.25, max_scale=1.0,  max_rotation=0)
	dis_ex = dis_out.eval(feed_dict={image: adv_robust})
	classify(dis_ex, correct_class=img_class, target_class=924)
